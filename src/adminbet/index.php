...
?>
		...
		<div class="row">
		<!--col-->
		 <div class="col-sm-6 col-md-3 margin-b-30 card">
				<div class="tile">
					<div class="tile-footer">
						 <i>بازدید کاربران در امروز</i>
					</div>
					<div class="tile-body clearfix">
						<span class="fa-fw select-all fas fa-xl"></span>
						<h4 class="pull-right">
							<?php echo $dpcn[0];?>
						</h4>
					</div>
                                    ...
                                    <ul class="list-group">
										<?php
											...
											foreach ( array_slice($row_NewUser, 0, 10) as $row_NewUsern ) {
												...
												// $avatar    = !empty($row_insta['avatar']) ? $row_insta['avatar'] : 'https://betboys.xyz/assets/default/live/images/support.jpg';
										?>
										<li class="list-group-item">
										
                                            <a href="plugins/users/user_manage.php?edit=<?= $row_NewUsern['id']; ?>"> <?= sanitize_output($row_NewUsern['first_name']). ' ' .sanitize_output($row_NewUsern['last_name']); ?></a>
                                        </li>
										<?php } ?>
                                    </ul>
                                </div>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><?php echo sanitize_output($row_tickets['id']); ?></td>
														<td>
															<a href="plugins/tickets/tickets_view.php?id=<?php echo $row_tickets['id']; ?>">
                                                    ...
                                                    <td>
														<?php
															if($row_tickets['status'] == 0){
																$status	=	'خوانده نشده';
															}elseif($row_tickets['status'] == 1){
																$status	=	'باز ';
															}
															else{
																$status	=	'بسته شده';
