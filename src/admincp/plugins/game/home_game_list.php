<?php 
   GLOBAL $content;
   $content['title']       = "لیست بازی های صفحه اصلی";
   require('../../layouts/sidebar.php');
   // Make unified connection variable
   
   
   $jsonResult = file_get_contents('../../../upload/API/upcoming/results.json');
                        ...
                        <table class="table table-bordered">
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td>
                                    <?php 
                                       ...
                                       echo $query_Recordstatus = "select result from matches where id='$match_id'";
                                       ...
                                       if($totalrowstatus>0){
                                        ...
                                        $matchstatus= $row_Recordstatus['result'];echo $matchstatus;
                                        if($matchstatus==NULL or !isset($matchstatus) or $matchstatus==""){ $matchstatus=0;}
                                       }
                                       
                                       ?>
                                    <ul class="icons-list">
                                       <?php if (in_array('home_game_status', $accccesslist)) {?>
										<li><a href="home_game_status.php?table=<?php echo encrypt('matches',session_id()."hsts"); ?>&field=<?php echo encrypt('result',session_id()."hsts"); ?>&id=<?php echo encrypt($match_id,session_id()."hsts"); ?>" data-popup="tooltip"><?= $matchstatus ==1 ? 'فعال' : 'غیرفعال' ?></a></li>
                                       <?php }?>
                                    </ul>
                                 </td>
                              </tr>
                              ...
                              <tr>
                                 ...
                                 <?php 
                                    mysqli_select_db($cn,$database_cn);
                                    $query_Recordstatus = "select result from matches where id='$match_id'";
                                    $Recordstatus = mysqli_query($cn,$query_Recordstatus) or die(mysqli_error($cn));
                                    $totalrowstatus=mysqli_num_rows($Recordstatus);
                                    $matchstatus=0;
                                    if($totalrowstatus>0){
                                     $row_Recordstatus = mysqli_fetch_assoc($Recordstatus);
                                     $matchstatus= $row_Recordstatus['result'];
                                     if($matchstatus==NULL or !isset($matchstatus) or $matchstatus==""){ $matchstatus=0;}
                                    }
