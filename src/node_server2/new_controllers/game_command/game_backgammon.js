...
var enter_game = require('./game_enter_game.js');
...
function status_16 ( data, callback ){
	...
	game.get_percent_play_with_game_id( data.game_id, function( get_row ){
			...
			affiliate( data.op_user_id, my_amount);
...
//			reset_data_backgammons( my_filter );
		})
	}
...
function status_4 ( data, callback){
...
//	if ( game_data.side == 'me' && game_data.time == 'WHITE' ){
		...
		get_my_and_op_user_info( data.user_id, data.op_user_id, function( result){
	
			if ( result != 0 ){
				data.my_data = result.my_data,
				data.op_data = result.op_data;
				
				game.get_my_and_op_data( data, function(row){
					data.my = row.my_data,
					data.op = row.op_data;
					...
					game.update_user_cash(data.user_id, -data.amount,data.game_id, '7', function(update){
								});
...
function get_my_and_op_user_info ( my_user_id, op_user_id, callback ){
	
	game.get_user_with_id( op_user_id, function(op_user){
						
		game.get_user_with_id( my_user_id, function(my_user){

			if ( op_user[0] != undefined && my_user[0] != undefined ){
				callback({my_data: my_user[0], op_data: op_user[0]});
			}
			else{
				callback(0);
			}
...
function reset_data_backgammons ( filter ){
