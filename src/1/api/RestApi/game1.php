...
require_once ROOT_PATH . '/newGames/db.php';
...
//		$this->hashid = new Hashids('Ghorayshi');
	...
	public function get_user_cash ( $id ){
		$db = new DB();
		$user = $db->multi_select('users', 'username, cash', ['id','status'], [$id, '1']);
		
		if ( $user == 0 )
			return false;
		
		return ['user_name'=> $user[0]['username'], 'user_cash' => $user[0]['cash']];
	}
...
//		
	...
	public function insert_transaction ( $invoice_type, $token, $amount){
		...
		$game = $this->get_game_name( $users['game_id'] );
