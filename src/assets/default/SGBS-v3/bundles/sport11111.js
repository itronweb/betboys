...
function prepareButtonEvents() {
	...
	$(document).on('click','.inplaybtn' ,function() {
        ...
        if ($(".betlist ul.bet").length < 21 && ($(".alertbox").addClass("hidden"), !$(this).hasClass("selected") && !$(this).hasClass("disabled"))) {
            ...
            var n = $(this).closest("tr.odddetails"),
                    t = n.data("eventid"),
                    i = n.data("marketid"),
                    h = $(this).data("runnerid"),
                    r = n.find("td .host").text(),
                    u = n.find("td .guest").text(),
                    f = lang == "fa" ? "نتیجه مسابقه" : "Match Result",
                    c = $(this).data("pick"),
                    l = $(this).data("points");
            n.hasClass("eventodd") && (r = $(this).closest("ul.odds").find(".host").val(), u = $(this).closest("ul.odds").find(".guest").val(), t = $(this).closest("div.eventodds").data("eventid"), i = $(this).closest("div.eventodds").data("marketid"), f = $(this).closest("tr.odddetails").parent().find(".inplayheader b").text());
...
function updateInplayOdds(UpdatedMatches) {
	...
	if(UpdatedMatches == '' ){
		window.location.replace("../bets/inplayBet");
	}
    var preMatches = $('table.inplaytable > tbody > tr.odddetails');
