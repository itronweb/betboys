function loadBetslipCookie() {
    ...
    if (typeof u != "undefined" && (t = JSON.parse(u), t.length > 0)) {
        for ($(".nobet").hide(), $(".bettotal").show(), r = "", i = 0; i < (t.length < 21 ? t.length : 20); i++)
            n = t[i], f = '<b class="team1 ellipsis ellipsis2">' + n.team1 + '<\/b><br /><b class="team2 ellipsis ellipsis2">' + n.team2 + "<\/b>", r += createBet(n.eventid, f, n.odd, n.pick, n.market, n.marketid, n.runnerid, n.points, n.sport_type);
...
function createBet(n, t, i, r, u, f, e, o, sport) {
    var s = '<ul data-eventid="' + n + '" data-pick="' + r + '" data-points="' + o + '" data-marketid="' + f + '" data-runnerid="' + e + '" data-bet-sport="' + sport + '"  class="bet">';
...
function saveBet2Cookie() {
    ...
    $("ul.inplay .betlist ul.bet").each(function() {
        if (findInArray(n, "runnerid", $(this).data("runnerid")) < 0) {
			...
			t.sport_type = $(this).data("bet-sport");
            n.push(t);
        }
    });
...
function prepareButtonEvents() {
	...
	$(document).on('click','.inplaybtn' ,function() {
//		console.log('click function');
        ...
        if ($(".betlist ul.bet").length < 21 && ($(".alertbox").addClass("hidden"), !$(this).hasClass("selected") && !$(this).hasClass("disabled"))) {
            ...
            var n = $(this).closest(".odddetails"),
                    ...
                    c = $(this).data("pick"),
                    l = $(this).data("points"),
					sport = $(this).data("bet-sport");
					
            n.hasClass("eventodd") && (r = $(this).closest(".odds").find(".host").val(), u = $(this).closest(".odds").find(".guest").val(), t = $(this).closest(".eventodds").data("eventid"), i = $(this).closest(".eventodds").data("marketid"), f = $(this).closest(".odddetails").parent().find(".inplayheader b").text());
            e = '<b class="team1 ellipsis ellipsis2">' + r + '<\/b><br /><b class="team2 ellipsis ellipsis2">' + u + "<\/b>";
            o = $(this).find("span").text();
            $(".selectedodds div.betlist").append(createBet(t, e, o, c, f, i, h, l,sport));
            saveBet2Cookie();
            renewDeleteEvent();
            prepareMultiple()
			...
			$('ul[data-runnerid="' + element + '"]').slideUp("slow", function() {
				var i = n.data("eventid"),
						r = n.data("marketid"),
						t = n.data("runnerid");
				$('.inplaybtn[data-runnerid="' + t + '"]').removeClass("selected");
				$(this).remove();
				prepareMultiple();
				calcTotal();
...
//                $(item).find('span.delete').trigger('click');
    ...
    if (UpdatedMatches !== undefined) {
        $.each(UpdatedMatches, function(match_id, item) {
            var preMatches = $('.odddetails');
			
            var row_event = $(".betslip ul[data-eventid=" + match_id + "]"),
                    suspend_status = 0,
...
//								console.log(item3	.type);
								if (suspend_status > 0 || item4.value == 0 ||(item.minute>80 && item3.type !='1x2') || (item.minute>40 && item3.type == 'First Half Goals') ) {
...
function updateInplayOdds(UpdatedMatches) {
    ...
    var preMatches = $('.odddetails');
    var Headers = $('.inplayheader');
    var soccerType = $('.sport-type');

//	$(Headers).addClass('leagueChange');
//	console.log(preMatches);
//	console.log(UpdatedMatches);
    $.each(Headers, function(i, item) {
        // check mikne age bazi ba data-league id mored nazar nabud league remove mikne
        if ($('.odddetails[leagueid="' + $(item).attr('data-leagueid') + '"]').length == 0)
            $('.inplayheader[data-leagueid="' + $(item).attr('data-leagueid') + '"]').fadeOut().remove();
    });

    $.each(soccerType, function(i, item) {
        if ($('.odddetails[sport-type="' + $(item).attr('data-sport') + '"]').length == 0)
            $('.sport-type[data-sport="' + $(item).attr('data-sport') + '"]').fadeOut().remove();
    });
...
//	console.log(UpdatedMatches);
    ...
    if (UpdatedMatches !== undefined) {
        ...
        $.each(UpdatedMatches, function(match_id, item) {
            ...
            // bazi jadid biad page reload mishe
            if ( row_event.length == 0){
                location.reload();
            }

            if ($("#page-name").val() == 'inplayOdds') {
...
//					console.log(item);
//					if( item.home_score > $('div.reasult').find('span').eq(0).text() || item.away_score > $('div.reasult').find('span').eq(2).text()){
...
//					}
					...
					if(item.sport == 'soccer')
					{
						...
						$('span.inplaytime').text((item.minute));
						
						// score
					$('div.mb-20').find('tr').eq(1).find('td').eq(1).text(item.home_score);
					$('div.mb-20').find('tr').eq(2).find('td').eq(1).text(item.away_score);
					// Yellow cards
					$('div.mb-20').find('tr').eq(1).find('td').eq(2).text(item.homeTeam.stats.yellowcards);
					$('div.mb-20').find('tr').eq(2).find('td').eq(2).text(item.awayTeam.stats.yellowcards);
					// Red cards
					$('div.mb-20').find('tr').eq(1).find('td').eq(3).text(item.homeTeam.stats.redcards);
					$('div.mb-20').find('tr').eq(2).find('td').eq(3).text(item.awayTeam.stats.redcards);
					// Corners
					$('div.mb-20').find('tr').eq(1).find('td').eq(4).text(item.homeTeam.stats.corners);
					$('div.mb-20').find('tr').eq(2).find('td').eq(4).text(item.awayTeam.stats.corners);
					// Penalties
							...
							if( item.status != 'FT' || item.status != 'CNCL' ){
								...
								$.each(odds_type.odds.data, function(odd_index1,odds_type1){
									...
									if(odds_type1.suspend == 0 && odds_type1.value !=0 ){
										...
										if(inplayOddsObject.find('.inplaybtn[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]').length == 0){
											  location.reload();
										}
										
									inplayOddsObject.find('.inplaybtn[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]').find('span').text(odds_type1.value);
										
									var test = $('div[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]');	
										
									var bet_btn = $('.inplaybtn[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]');	
										
									var btn_list = inplayOddsObject.find('li[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]');
										
									bet_btn.removeClass('disabled lock');
									bet_btn.find('span').removeClass('fa fa-lock lock');
										
									var prev_odd = bet_btn.attr('data-prev_odd');
										
									if (prev_odd > odds_type1.value)
										bet_btn.find('span').prepend('<i class="fa-caret-down down-arrow fa"></i>');
									else if (prev_odd < odds_type1.value)
										bet_btn.find('span').prepend('<i class="fa-caret-up up-arrow fa"></i>');
										
									bet_btn.attr('data-prev_odd', odds_type1.value);

									}
									else{
										
										var btn_btn = inplayOddsObject.find('.inplaybtn[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]');
										
										btn_btn.find('span').text(' ');
										btn_btn.addClass('disabled  lock');
										btn_btn.find('span').addClass('fa fa-lock lock');
...
//					row_event.find('span.inplaytime').text('پایان نیمه اول' );
				...
				else if(item.status == 'FT'){
					row_event.find('input.eninplaytime').val('FT');
				}
                else{
					row_event.find('span.inplaytime').text(" ' " + item.minute);
                    row_event.find('input.eninplaytime').val(item.period);
				}
...
//                console.log(item.home_score);
                ...
                $.each(item.odds.data[0].types.data, function(index3, item3) {
                    if(item3.type == 'Fulltime Result' || item3.type == '1x2' ){
						...
						// if(row_event.attr('data-favorite') == 'true'){
						// 	$('.inplayheaders[data-leagueid=1]').removeClass('leagueChange');
						// 	$('.inplayheaders[data-leagueid=1]').removeClass('hidden');
						// }
						// else{
						// 	if(!row_event.hasClass('hidden'))
						// 		inplayHeader.removeClass('leagueChange');
						// }
							
//						console.log(inplayHeader);
...
//	console.log(UpdatedMatches);
    $.each(Headers, function(i, item) {
        if ($('.odddetails[data-league="' + $(item).attr('data-league') + '"]').length == 0)
            $('.inplayheader[data-league="' + $(item).attr('data-league') + '"]').remove();
    });
...
//						
						...
						$.each(item.odds.data[0].types.data, function(odd_index,odds_type){
							if((((item.minute < 80 && odds_type.type != '1x2') ) || (odds_type.type == '1x2'))  && (item.status != 'FT' || item.status != 'CNCL') ){
								if((item.minute < 40 && odds_type.type == 'First Half Goals') || odds_type.type != 'First Half Goals' ){
								var inplayOddsObject = $('div[data-eventname="'+odds_type.type+'"]'),suspend_status = 0;
								if(inplayOddsObject.length == 0)
									location.reload();
...
//								console.log(inplayOddsObject);
		...
		//							console.log(inplayOddsObject.find('li[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]').find('span'));
									if(odds_type1.suspend == 0 && odds_type1.value !=0 ){
										if(inplayOddsObject.find('li[data-runnerid="'+item.id+'-2-'+odds_type.type+'-'+odds_type1.label+'"]').length == 0){
											location.reload();
										}
...
function updateUpcoming(n) {
    $.each(n, function(n, t) {
        var i = $('div[data-eventid="' + t.EventId + '"]'),
                o, e, u, r, f;
        t.Command == "Update" ? (i.find(".inplayscore").text(toFarsi(t.Score1 + " - " + t.Score2)), o = Math.round(t.GameTime + (new Date(t.CurrentTime) - new Date(t.LastTimeUpdate)) / 1e3), e = secondsTimeSpanToMS(o, t.Eventstatus), i.find(".eninplaytime").val(e), i.find(".inplaytime").text(toFarsi(e))) : t.Command == "Add" && i.length == 0 ? (u = $('div[data-leagueid="' + t.LeagueId + '"]'), u.length == 0 && (r = '<div class="inplayheader" data-leagueid="' + t.LeagueId + '" data-rankid="' + t.LeagueRankID + '">', r += '<th><span class="match"><b>' + t.LeagueName + "<\/b><\/span><\/th>", r += '<th style="width:44px"><b>1<\/b><\/th>', r += '<th style="width:44px"><b>X<\/b><\/th>', r += '<th style="width:44px"><b>2<\/b><\/th>', r += '<th style="width:25px"><\/th>', r += "<\/div>", $(".inplayheader").each(function() {
            if (parseInt($(this).data("rankid")) > t.LeagueRankID)
                return $(this).before(r), u = $('div[data-leagueid="' + t.LeagueId + '"]'), !1
        }), u.length == 0 && ($(".inplaytable").append(r), u = $('div[data-leagueid="' + t.LeagueId + '"]'))), f = '<div data-eventid="' + t.EventId + '" data-marketid="' + t.MarketId + '" class="odddetails">', f += '<div><span class="fa fa-clock-o timer"><\/span><b class="host ellipsis">' + t.Team1 + '<\/b><span class="scoredash">-<\/span><b class="guest ellipsis">' + t.Team2 + '<\/b>\n<span class="inplaytime2">' + getTime(t.StartAt) + "<\/span><\/td>", f += '<div style="padding:0" colspan="3"><\/td>', f += '<td><a class="has-tip fa fa-plus-circle more" href="/Sport/PreEvent/' + t.EventId + '" title="' + ($("#lang").val() == "fa" ? "شرط های بیشتر" : "More Markets") + '"><\/a><\/div>', f += "<\/div>", u.after(f)) : t.Command == "Remove" && (i.prev().hasClass("inplayheader") && (i.next().hasClass("inplayheader") || i.next(".inplayheader").length == 0) && i.prev().remove(), i.remove(), $(".totalevents").text(toFarsi($(".odddetails").length)))
    });
    $(".totalevents").text(toFarsi($(".odddetails").length));
    $(".has-tip").frosty()
}
...
$(document).ready(function() {
    ...
    $(".deleteall").click(function() {
        ...
        $(".betlist ul").remove();
        $(".odds .inplaybtn").removeClass("selected");
        calcTotal();
        $(".nobet").show();
        $(".bettotal").hide();
    ...
    $(".placebet").unbind().click(function(n) {
        ...
        if (myItem.length == 1) {
            jsonObject.data.push({
				...
				"sport_type": myItem.data("bet-sport"),
            });
            ...
            myItem.each(function(i) {
                jsonObject.data.push({
					...
					"sport_type": $(this).data("bet-sport"),
                });
            });
        }
        ...
        n.handled !== !0 && (n.handled = !0, n.stopImmediatePropagation(), $(this).prop("disabled", !0), $(this).addClass("disabled in-progress"), $(".alertbox").addClass("hidden"), parseFloat($(".totalstake").html().replace("$", "").replace(/,/g, "")) > 0 ? $.post(action, {
            ...
            switch (n.result) {
                ...
                case "Expired2":
                    ...
                    break;
				case "Multi_sport":
                    $(".alertbox").html(lang == "fa" ? "امکان پیش بینی در چند ورزش مختلف وجود ندارد." : "This bet slip has been timed out.").addClass("errorbox").removeClass("hidden");
                    break;
                case "LowBalance":
