...
class Bets extends Public_Controller{
	...
	public function inplayBets(){
		...
		if(($now_time - $inplay_time) >= 5){
		    ...
		    file_put_contents(APPPATH . 'logs/API/inplay_api.txt', file_get_contents('https://soccer.sportmonks.com/api/v2.0/livescores/now?api_token=' . API_KEY . '&include=odds,inplayOdds,localTeam,visitorTeam,league'));
		}
				...
				if($gotBreak === false){
					...
					foreach($value as $match){
						if($match->time->status == 'LIVE' OR $match->time->minute <= 89){
							...
							$match_inplay_odds = $match->inplayOdds->data[0];
							...
							if((!is_array($last) AND !is_object($last)) OR count((array)$last) <= 0){
								...
								$first_odd = isset($last->data[0]->value) ? $last->data[0]->value : $last->values[0]->value;
								$second_odd = isset($last->data[1]->value) ? $last->data[1]->value : $last->values[1]->value;
								$third_odd = isset($last->data[2]->value) ? $last->data[2]->value : $last->values[2]->value;
	...
	public function moreBets($match_id, $type){
		...
		$live_address = file_get_contents('http://medal2.world/sport/event?id=100000' . $game_id . '&type=live');
		...
		if(!in_array($status, array('FT', 'AET', 'FT_PEN')) AND !empty($sorted_match_odds) AND (is_array($sorted_match_odds) OR is_object($sorted_match_odds))){
			...
			foreach($sorted_match_odds as $key => $value){
				...
				if($odd_name_lower == '3way result'){
					$matches_to_odds = file_get_contents(APPPATH . 'logs/API/inplay_api.txt');
					$matches_to_odds = json_decode($matches_to_odds, false);
					$current_match = array();
					foreach($matches_to_odds->data as $match_to_odds){
						if($match_to_odds->id == $match_id){
							$current_match = $match_to_odds;
							break;
						}else{
							continue;
						}
					...
					if(count($current_match) >= 1){
						$match_odds = $current_match->odds->data[0]->bookmaker->data[0]->odds;
						$match_inplay_odds = $current_match->inplay->data[0]->bookmaker->data[0]->odds;
						$last = (isset($match_inplay_odds) AND (is_array($match_inplay_odds) OR is_object($match_inplay_odds))) ? $match_inplay_odds : ((isset($match_odds) AND (is_array($match_odds) OR is_object($match_odds))) ? $match_odds : array());
						if((!is_array($last) AND !is_object($last)) OR count((array)$last) <= 0){
							continue;
						}else{
							...
							if(($current_match->scores->localteam_score - $current_match->scores->visitorteam_score) == 1){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$first_odd = $first_odd / 2;
									$first_odd = round($first_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
									$first_odd = $first_odd / 2.25;
									$first_odd = round($first_odd, 2);
								}elseif($current_match->time->minute >= 46 AND $current_match->time->minute < 70){
									$first_odd = $first_odd / 2.5;
									$first_odd = round($first_odd, 2);
								}elseif($current_match->time->minute >= 70){
									$first_odd = 1.1;
								}
							}elseif(($current_match->scores->localteam_score - $current_match->scores->visitorteam_score) == 2){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$first_odd = $first_odd / 2.5;
									$first_odd = round($first_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							}elseif(($current_match->scores->localteam_score - $current_match->scores->visitorteam_score) >= 3){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$first_odd = $first_odd / 3;
									$first_odd = round($first_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							if($current_match->scores->localteam_score == $current_match->scores->visitorteam_score){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$second_odd = $second_odd / 2;
									$second_odd = round($second_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							if(($current_match->scores->visitorteam_score - $current_match->scores->localteam_score) == 1){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$third_odd = $third_odd / 2;
									$third_odd = round($third_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							}elseif(($current_match->scores->visitorteam_score - $current_match->scores->localteam_score) == 2){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$third_odd = $third_odd / 2;
									$third_odd = round($third_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							}elseif(($current_match->scores->visitorteam_score - $current_match->scores->localteam_score) >= 3){
								if($current_match->time->minute >= 0 AND $current_match->time->minute < 30){
									$third_odd = $third_odd / 2;
									$third_odd = round($third_odd, 2);
								}elseif($current_match->time->minute >= 30 AND $current_match->time->minute < 45){
							...
							$third_odd = $third_odd == 0 ? 0 : $third_odd;
							$third_odd = (($third_odd < 1 AND $third_odd > 0 AND $third_odd != 0) OR $third_odd == 1) ? 1.1 : $third_odd;
							$value->bookmaker->data[0]->odds->data[0]->value = $first_odd;
							$value->bookmaker->data[0]->odds->data[1]->value = $second_odd;
							$value->bookmaker->data[0]->odds->data[2]->value = $third_odd;
						}
					}
				}
				...
				foreach($value->bookmaker->data[0]->odds->data as $odd){
					...
					$disable = 'false';
					if($matchDetails->time->minute >= 45){
			            if($odd_name_lower == '3way result 1st half' OR $odd_name_lower == 'over under 1st half' OR $odd_name_lower == 'ht ft double' OR $odd_name_lower == 'both teams to score - 1st half' OR $odd_name_lower == 'correct score 1st half' OR $odd_name_lower == 'double chance - 1st half' OR $odd_name_lower == 'odd even 1st half'){
			                $disable = 'true';
			            }else{
			                $disable = 'false';
			            }
					}elseif($odd_name_lower == 'both teams to score'){
					    if($matchDetails->scores->visitorteam_score >= 1 AND $matchDetails->scores->localteam_score >= 1){
					        $disable = 'true';
					    }else{
					        $disable = 'false';
					    }
					}elseif($odd_name_lower == 'home team score a goal'){
					    if($matchDetails->scores->localteam_score >= 1){
					        $disable = 'true';
					    }else{
					        $disable = 'false';
					    }
					}elseif($odd_name_lower == 'away team score a goal'){
					    if($matchDetails->scores->visitorteam_score >= 1){
					        $disable = 'true';
					    }else{
					        $disable = 'false';
					    }
					...
					$result .= '<a data-eventid="' . $matchDetails->id . '" data-runnerid="' . $matchDetails->id . '-' . $odd->value . '-' . $odd_name . '-' . $odd_label . '" data-pick="' . $pick . '" data-points="" class="' . ($disable == 'true' ? 'passive-ma ' : '') . 'inplaybtn eventodd odd-link odd-sub-button odd-' . ((count($value->bookmaker->data[0]->odds->data) % 3) == 0 ? 'triple' : 'double') . '" href="javascript:;"><div class="odd-title"><span>' . $odd_title . '</span></div><div class="odd-rate odd-main-button"><span>' . ($disable == 'true' ? '...' : $odd->value) . '</span></div></a>';
				}
