...
//            die('Access Denied from the HTTP Requests');
    ...
    public function GetUpcomingOdds () {
        ...
        $matchs_error = false;
        $resultsToday_error = false;
        try {
            $matches = $this->client->get($this->destination.'home/matches.json');    
            // $resultsToday = $client->get('http://pishgu.com/demo/upload/API/home/resultsToday.json'); 
            // $matches = $client->get('http://pishgu.com/demo/sample.json');    
        }
        ...
        if(!$matchs_error) {
            file_put_contents(API_DIR . "home/matches.json" , $matches->getBody()->getContents());
        }



        try {
            $resultsToday = $this->client->get($this->destination.'home/resultsToday.json');    
            // $resultsToday = $client->get('http://pishgu.com/demo/upload/API/home/resultsToday.json'); 
            // $matches = $client->get('http://pishgu.com/demo/sample.json');    
        }
        catch (GuzzleHttp\Exception\ClientException $e) {
            $resultsToday_error = true;
            $response = $e->getResponse();
            $responseBodyAsString = $response->getBody()->getContents();
        }

        if(!$resultsToday_error) {
            file_put_contents(API_DIR . "home/resultsToday.json" , $resultsToday->getBody()->getContents());
        }



        if(!$matchs_error && !$resultsToday_error) die(json_encode(['success' => 1 ]));
        // $include = array(
        //     'competition' ,
        //     'homeTeam' ,
        //     'awayTeam' ,
    ...
    public function GetInplayOdds () {
        ...
        try {
            $inPlay = $this->client->get($this->destination.'inPlay/matches.json');    
            // $inPlay = $client->get('http://pishgu.com/demo/upload/API/inPlay/matches.json'); 
            // $matches = $client->get('http://pishgu.com/demo/sample.json');    
        }
        catch (GuzzleHttp\Exception\ClientException $e) {
            $inPlay_error = true;
        ...
        if(!$inPlay_error) {
            file_put_contents(API_DIR . "inPlay/matches.json" , $inPlay->getBody()->getContents());
        }

        if(!$inPlay_error) die(json_encode(['success' => 1 ]));
    ...
    public function getDateRangeMatches ( $day = 0 ) {
        ...
        for ($d=0;$d<5; $d++) {
            $dayError = false;
            try {
                $dayGames = $this->client->get($this->destination.'upcoming/day_'.$d.'.json');    
                $dayResults = $this->client->get($this->destination.'upcoming/results'.$d.'.json');
                // $inPlay = $client->get('http://pishgu.com/demo/upload/API/inPlay/matches.json'); 
                // $matches = $client->get('http://pishgu.com/demo/sample.json');    
            }
            catch (GuzzleHttp\Exception\ClientException $e) {
            ...
            if(!$dayError) {
                file_put_contents(API_DIR . "upcoming/day_$d.json" , $dayGames->getBody()->getContents());
                file_put_contents(API_DIR . "upcoming/results$d.json" , $dayResults->getBody()->getContents());
                $day_games = json_decode(file_get_contents(API_DIR . "upcoming/day_$d.json"),true);
                foreach ($day_games['data'] as $key => $value) {
                    $all_days['data'][] = $value;
                }
        ...
        if(!$total_error) {
          ...
          file_put_contents(API_DIR . "home/matches.json" , json_encode($all_days));
          file_put_contents(API_DIR . "inPlay/matches.json" , json_encode($all_days));
          die(json_encode(['success' => 1 ])); 
...
//        $matches = $this->CI->soccerama->livescores($resultInclude)->today();
		...
		$matches = json_decode(file_get_contents(API_DIR . "upcoming/results0.json"));
		var_dump($this->checkResultUpComing($matches));
...
//        dd($matches);
    ...
    public function UpcomingResultDayBeforeYesterday () {
        ...
        $matches = $this->CI->soccerama->matches($resultInclude)->oneDate(date('Y-m-d' , strtotime("-2 days")));
        return $this->checkResultUpComing($matches);
    }

	public function test(){
		$this->UpcomingResultToday();
//		var_dump($this->UpcomingResultYesterday());
//		$this->UpcomingResultDayBeforeYesterday();
//		$this->checkResultUpComing('9587380');
	}
    /**
