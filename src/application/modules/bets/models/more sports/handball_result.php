...
class Handball_result{
	...
	*/
	...
	public function check_1X2_1st_inning ( $match, $form ){
		$home = $match[$form->match_id]->homeTeam->t1;
		$away = $match[$form->match_id]->awayTeam->t1;
		
		return $this->check_win_result( $form->pick, $home, $away );
	}
	
	public function check_1X2_2nd_inning ( $match, $form ){
		$home = $match[$form->match_id]->homeTeam->t2;
		$away = $match[$form->match_id]->awayTeam->t2;
