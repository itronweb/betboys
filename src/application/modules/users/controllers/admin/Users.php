...
class Users extends Admin_Controller {
        ...
        if ( $this->formValidate(false) ) {
            ...
            if ( $reminder->exists($User) ) {
                if ( $reminder->complete($User , $code , $password) )
                    $this->message->set_message('رمز عبور شما با موفقیت تغییر یافت. می توانید با رمز عبور جدید وارد پنل کاربری خود شوید.' , 'success' , 'تغییر رمز عبور' , ADMIN_PATH . '/users/login')->redirect();
                else
                    $this->message->set_message('کد وارد شده اشتباه است' , 'warning' , 'تغییر رمز عبور' , ADMIN_PATH . '/users/users/resetPasswordBySms/' . $user_id)->redirect();
            }
            else {
                $this->message->set_message('کد وارد شده معتبر نیست. لطفا مجدد تلاش کنید' , 'fail' , 'تغییر رمز عبور' , ADMIN_PATH . '/users/login')->redirect();
            }
        }
    ...
    public function withdraw () {
        ...
        $this->smart->assign([
            'withdraw' => $all ,
            'title' => 'درخواست جایزه' ,
			'totalStake' => $_SESSION['totalStake'],
        ]);
