...
require_once 'lib/Newgame/roulette.php';
require_once 'lib/Newgame/slot.php';
	...
	//	
		...
		if ( $game->check_this_bet_is_valid ($bets['all_bets'], $Data['uid']) ){
			...
			if ( $bets['win_bets'] > 0 ){
				...
				$game->update_affiliate_user_cash($Data['uid'],$bets['all_bets'],$Data['game_id']);
	...
	elseif($game_name === 'slot'){
		...
		if ( $game->check_this_bet_is_valid_with_token( $bets, $Data['token']) || $free_game === true ) {
            ...
            if ( $free_game === false ){
                ...
                $game->insert_transaction( 192 ,$Data['token'], $bets);
            }
//
			...
			else if ( $free_game === false) {
				$game->update_affiliate_user_cash($Data['uid'],$bets,$Data['game_id']);
			}
